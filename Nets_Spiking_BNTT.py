...
class HYBRID16_TBN(nn.Module):
    def __init__(self, dt=0.001, t_end=0.100, inp_rate=100, grad_type='Linear', thresh_init_wnorm=False,
        ...
        batchnormlist = [self.bn1_list, self.bn1_1_list, self.bn3_list, self.bn5_list]
...
class SNN_VGG9_TBN(nn.Module):
    def __init__(self, dt=0.001, t_end=0.100, inp_rate=100, grad_type='Linear', thresh_init_wnorm=False,
                 leak_mem=0.99, img_size=32, inp_maps=3, c1_maps=64, c2_maps=64, ksize=3, fc0_size=200,
                 num_cls=1000, drop_rate=0.5, use_max_out_over_time=False, timesteps = 10,
                 inp_type = "aps", encoder_decoder = False):
        super(SNN_VGG9_TBN, self).__init__()

        # ConvSNN architecture parametersi
        ...
        self.up_conv_1 = nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1, bias=bias_flag)
        self.deconv_1 = deconv(128, 64, False)
        self.deconv_2 = deconv(64, 1, False )
    ...
    def forward(self, inp):
        ...
        # If normal arch return the accumulated membrane potential of fc2
        if self.encoder_decoder == False:
            out_voltage = mem_fc2
            out_voltage = (out_voltage) / self.num_steps
            return out_voltage
        else: # Encoder decoder arch: build decoder on top of conv6 output
            encoder_out = mem_conv4
            encoder_out = (encoder_out) / self.num_steps
            x = self.up_conv_1(encoder_out)
            x = self.deconv_1(x)
            x = self.deconv_2(x)
            return x
